...
export default function LayoutEstudiante() {
  ...
  return (
    <div className={`min-h-screen ${tema === 'dark' ? 'bg-[#090d16] text-slate-100' : 'bg-slate-50 text-slate-900'} flex flex-col md:flex-row font-sans selection:bg-blue-600 selection:text-white relative overflow-hidden transition-colors duration-200`}>
      ...
      <aside className={`hidden md:flex md:w-72 ${tema === 'dark' ? 'bg-[#0e1526] border-slate-800/80' : 'bg-white border-slate-200'} border-r flex-col shrink-0 relative z-30 transition-colors duration-200`}>
        ...
        <div className={`p-5 border-b ${tema === 'dark' ? 'border-slate-800/80' : 'border-slate-200'} space-y-4`}>
          ...
          <Link
            ...
            className={`p-3 rounded-xl ${
              tema === 'dark' ? 'bg-[#090e1a] border-slate-800 hover:border-slate-700' : 'bg-slate-50 border-slate-200 hover:border-slate-300'
            } border flex items-center justify-between transition-all group cursor-pointer`}
          >
            <div className="flex items-center space-x-3 overflow-hidden">
              <div className={`w-8 h-8 rounded-lg ${tema === 'dark' ? 'bg-slate-800 text-slate-200' : 'bg-white text-slate-800'} border border-slate-700/50 flex items-center justify-center font-bold text-xs shrink-0 shadow-sm`}>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.ruta}
                to={item.ruta}
                className={`group flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold transition-all duration-150 cursor-pointer ${
                  estaActivo
        ...
        {/* Footer Sidebar */}
        <div className={`p-4 border-t ${tema === 'dark' ? 'border-slate-800/80 bg-[#090e1a]' : 'border-slate-200 bg-slate-50'} space-y-1.5`}>
          <button
            type="button"
          ...
          {userRole === "Administrador" && (
            <button
              ...
              className="w-full flex items-center space-x-2.5 px-3 py-2 rounded-lg text-xs font-semibold text-purple-400 bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/20 transition-all cursor-pointer"
            >
          ...
          <button
            ...
            className="w-full flex items-center space-x-2.5 px-3 py-2 rounded-lg text-xs font-medium text-rose-500 hover:bg-rose-500/10 transition-all cursor-pointer"
          >
      ...
      <main className={`flex-1 flex flex-col min-w-0 pb-24 md:pb-0 overflow-y-auto ${tema === 'dark' ? 'bg-[#090d16]' : 'bg-slate-50'} relative z-10 transition-colors`}>
        ...
        <header className={`hidden md:flex items-center justify-between px-8 py-3.5 ${
        ...
        } backdrop-blur-md border-b sticky top-0 z-20 transition-colors`}>
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              type="button"
              onClick={alternarTema}
              className={`px-3 py-1.5 rounded-lg border text-xs font-bold transition-all cursor-pointer flex items-center space-x-2 ${
                tema === 'dark'
